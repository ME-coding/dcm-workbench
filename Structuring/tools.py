...
# ============================================================
# Helpers — Schedules / Pricing utilities
# ============================================================
...
def build_amortization(
    ...
    """
    Small amortization schedule generator (same logic as in the Pricer).
    ...
    rate_annual : float
        Annual interest rate in decimal (e.g., 0.04 for 4%).
    years : float
        Maturity in years.
    freq : int
        Payments per year (1, 2, 4, ...).
    structure : {"bullet", "equal_principal", "annuity"}
        Type of amortization profile.

    Returns
    -------
    pd.DataFrame
        Schedule with period index, time in years, cashflows,
        and outstanding amounts (begin/end).
    """
...
# Optional dependency (docxtpl) for Word document rendering
try:
...
def _render_termsheet_docx(template_path: str, context: Dict[str, Any]) -> bytes:
    """
    Render a Word file from a docxtpl template and a Jinja2 context.
    Returns raw bytes of the .docx.
    """
    if not DOCTPL_OK:
...
def _ytm_from_price(
    ...
    ----------
    clean_price_pct : float
        Clean price as % of par (e.g., 99.50).
    coupon_rate_pct : float
        Annual coupon rate in % (e.g., 4.00).
    years_to_mty : float
        Years from settlement to maturity.
    freq : int
        Coupons per year (1, 2, 4).
    redemption : float
        Redemption value at maturity per 100 par.
...
def tool_termsheet():
    """UI — Term Sheet Builder."""
    st.markdown("#### Term Sheet Builder")

    # --- Inputs (align with .docx placeholders) ---
    ...
    with c2:
        ...
        issue_amount = st.number_input(
            "Issue amount (notional)",
            min_value=100000.0, value=500_000_000.0,
    ...
    with c3:
        # Coupon rate + frequency (used for {{coupon}} and yield calc)
        coupon_rate = st.number_input(
            "Coupon rate (% p.a.)", min_value=0.0, value=4.00, step=0.10,
        ...
        clean_price = st.number_input(
            "Clean price (% of par)", min_value=0.0, value=99.50, step=0.10,
            format="%.2f", key="ts_clean"
        )
        accrued = st.number_input(
            "Accrued (% of par)", min_value=0.0, value=0.50, step=0.10,
            format="%.2f", key="ts_accrued"
        )
        issue_price = st.number_input(
            "Issue price (% of par)", min_value=0.0, value=100.00, step=0.10,
    ...
    # --- Auto-compute {{yield}} from clean price / coupon / freq ---
    ...
    # Build {{coupon}} display string
    coupon_str = f"{coupon_rate:.3f}% ({coupon_freq_label})"

    # --- Context for docxtpl (keys must match .docx placeholders) ---
    context: Dict[str, Any] = {
        ...
        "governing_law ": governing_law.strip(),  # safety for trailing-space tag in Word
...
# ============================================================
# TOOL 2 — Amortization Builder (Interactive) — Single version
# ============================================================

def tool_amortization():
    """
    UI — Amortization Builder.
    This version places the chart title outside (as markdown) and uses a right axis for the line.
    """
