...
def render():
    ...
    with colTR:
        ...
        st.markdown(
            """
            Convertible bonds can be viewed as a straight bond plus an embedded conversion option,
            with an investor put and an issuer call. Because multiple continuous decisions can occur
            over time, pricing is path-dependent.
            """
        )
        st.markdown(
            """
            **Definitions**
            ...
            - *Conversion option*: the right for the bondholder to exchange each bond for a predetermined number
              of the issuer’s shares, according to the conversion ratio.
              The parity (the current equity value of one bond if converted) equals the current share price (S₀)
        ...
        st.markdown(
            ...
            **Approach (binomial)**
            ...
            3) At each node, apply the optimal decision: continue, convert, exercise the put, or (if called) settle at the
               better of conversion vs call price. Backward induction yields today’s price.
            """
        )
    ...
    spec = ConvSpec(
        ...
        call_start_year=call_start,
        putable=True,
        put_price=put_price,
        put_start_year=put_start,
        steps=steps,
    )
    ...
    with colBL:
        ...
        ).encode(x="Stock (S):Q", y=alt.value(0), text="label:N")
        ...
        ref_rules = alt.Chart(ref_lines_df).mark_rule(strokeDash=[3, 4], strokeWidth=1.2).encode(
            ...
            color=alt.Color(
                "label:N",
                scale=alt.Scale(domain=["Call price", "Put price"], range=["#ef4444", "#10b981"]),
        ...
        chart = alt.layer(
            ...
            ref_rules,
            ref_text,
            *( [be_layer] if be_layer is not None else [] )
        ).properties(height=420).interactive(bind_y=False)
        ...
        st.markdown(
            ...
            <span style="color:#2563eb; font-weight:600;">Convertible price</span> (blue) rises with the stock and is floored by the straight bond value (grey).
            The <span style="color:#f4d03f; font-weight:600;">parity</span> (yellow, dashed) is the immediate equity value upon conversion (<code>S × ratio</code>).
            The orange dot marks the break-even where conversion becomes economically indifferent (price ≈ parity).
            Dashed green/red lines show investor <em>put</em> and issuer <em>call</em> cash settlement references.
            </div>
            """,
            unsafe_allow_html=True,
        )
